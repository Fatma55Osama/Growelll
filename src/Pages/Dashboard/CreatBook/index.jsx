import React, { useRef, useState } from 'react'
import styles from './index.module.css'
import { InputAdornment } from '@mui/material'
import { IoIosArrowRoundBack } from 'react-icons/io'
import { Link } from 'react-router-dom'
import { getData } from '../../../data/Repo/getData'
import axios from 'axios'
import Swal from 'sweetalert2'
export default function CreatBook() {

    const bookTitle = useRef()
    const description = useRef()
    const bookUrl = useRef()
    const aboutOfBook = useRef()
    const BookImage = useRef()
    const [error, setError] = useState()

    let tokenDoctor = localStorage.getItem("tokenDoctor") || sessionStorage.getItem("tokenDoctor");

    const handelCreateBook = (e) => {
        e.preventDefault();

        let bookTitlevalue = bookTitle.current.value;
        let descriptionvalue = description.current.value;
        let bookUrlvalue = bookUrl.current.value;
        let aboutOfBookvalue = aboutOfBook.current.value;
        let BookImagevalue = BookImage.current.files[0];

        if (!bookTitlevalue || !descriptionvalue || !bookUrlvalue || !aboutOfBookvalue || !BookImagevalue) {
            Swal.fire({
                icon: "warning",
                text: "All fields are required"
            });
            return;
        }

        const formData = new FormData();
        formData.append("bookTitle", bookTitlevalue);
        formData.append("description", descriptionvalue);
        formData.append("bookUrl", bookUrlvalue);
        formData.append("aboutOfBook", aboutOfBookvalue);
        formData.append("BookImage", BookImagevalue);

        axios.post('https://localhost:7071/api/BookEvent/CreateBookEvent', formData, {
            headers: {
                Authorization: `Bearer ${tokenDoctor}`,
                'Content-Type': 'multipart/form-data',
            }
        }).then((res) => {
            console.log("create Book", res);
            Swal.fire({
                icon: 'success',
                title: 'Submitted Successfully Book!',
                text: 'Your request has been sent.',
                confirmButtonColor: '#3085d6'
            });
            bookTitle.current.value = '';
            description.current.value = '';
            bookUrl.current.value = '';
            aboutOfBook.current.value = '';
            BookImage.current.value = '';
            setError('');
        }).catch((err) => {
            console.error("create Book error", err);
            if (err.response && err.response.data) {
                const validationErrors = err.response.data.errors;
                if (validationErrors) {
                    console.log("Validation errors:", validationErrors);
                    setError(validationErrors);
                } else {
                    setError({ general: [err.response.data.message || "Unknown error occurred"] });
                }
            }
            Swal.fire({
                icon: 'error',
                title: 'Submission Failed',
                text: 'Please try again later or check your inputs.',
                confirmButtonColor: '#d33'
            });
        });
    };

    return (
        <div className='col-12' id={styles.parent}>
            <div className='container  d-flex justify-content-center ' id={styles.create}>
                <div className='col-10 d-flex flex-column'>
                    <Link to={'/bookdoctor'}>  <IoIosArrowRoundBack className='mb-3 text-black' style={{ fontSize: "50px" }} /></Link>
                    <h2>Create Book</h2>
                    <form action="" className='col-12 mt-5 d-flex flex-column gap-5' onSubmit={handelCreateBook}>
                        <div className='col-12 d-flex flex-row justify-content-between flex-wrap gap-5 gap-md-0'>

                            <div className='col-12 col-md-12 d-flex flex-column gap-2'>
                                <label>Book Title</label>
                                <input ref={bookTitle} className='py-2 col-12 col-md-11' type="text" placeholder='  Please Enter your Book Title ' />
                                {error?.BookTitle && <div className="text-danger">{error?.BookTitle[0]}</div>}
                            </div>



                        </div>

                        <div className='col-12 d-flex flex-row justify-content-between flex-wrap gap-3 gap-md-0'>

                            <div className='col-8 col-md-6 d-flex flex-column gap-2'>
                                <label>Description</label>
                                <input ref={description} className='py-2 col-12 col-md-10' type="text" placeholder='  Please Enter your description ' />
                            </div>

                            <div className='col-10 col-md-6 d-flex flex-column gap-2 '>
                                <label>Book Url</label>
                                <input ref={bookUrl} className='py-2 col-10' type="text" placeholder='Please Enter your book URL' />
                                {error?.BookUrl && <div className="text-danger">{error?.BookUrl[0]}</div>}

                            </div>
                        </div>
                        <div className='col-12 d-flex flex-row justify-content-between flex-wrap gap-3 gap-md-0'>

                            <div className='col-8 col-md-6 d-flex flex-column gap-2'>
                                <label>About Of Book</label>
                                <input ref={aboutOfBook} className='py-2 col-12 col-md-10' type="text" placeholder='  Please Enter your About Of Book ' />
                                {/* {error?.NumberOfQuestions && <div className="text-danger">{error?.NumberOfQuestions[0]}</div>} */}

                            </div>

                            <div className='col-8 col-md-6 d-flex flex-column gap-2'>
                                <label>Book Image</label>
                                <input ref={BookImage} className='py-2 col-12 col-md-10' type="file" placeholder='  Please Enter your Book Image  ' />

                            </div>

                        </div>

                        <div className='d-flex justify-content-center'>

                            <button type='submit' className={styles.button + ' py-2 py-md-3 col-5 text-white col-lg-3 '}>Save Book </button>
                        </div>

                    </form>
                </div>

            </div>
        </div>
    )
}
